...
class EditorSectionList:
    def __init__(self, parent, existing_sections, callback) -> None:
        ...
        if len(existing_sections) > 0:
            for index, section in enumerate(existing_sections):
                self.components.append(EditorSection(self.parent, self, index + 1, section.name, section.reading_time, section.hours, section.minutes))
    ...
    def validate_entries(self, *args):
        ...
        for component in self.components:
            if (component.name_var.get() == "") or ((component.hours_var.get() + component.minutes_var.get()) == 0):
    ...
    def apply(self):
        ...
        reading_time_list = [component.reading_time_var.get() for component in self.components]
...
class EditorSection:
    def __init__(self, parent, controller, section_number, name="", reading_time=0, hours=0, minutes=0) -> None:
        """
        ...
        self.reading_time_entry = ttk.Entry(
            self.canvas,
            bootstyle="dark",
            textvariable=self.reading_time_var
        )
