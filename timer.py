...
if __name__ == "__main__":
    window = ttk.Window(themename="robin")
    window.geometry("1920x1080")
    window.configure(bg="#FFFFFF")

    subjects = [
        Subject("Spanish B SL", 0),
        Subject("Spanish A Language & literature", 0),
        Subject("Spanish A Literature", 0),
    ]
    subjects[0].sections.append(Section("Paper 1", 1, 15))
    subjects[0].sections.append(Section("Paper 2", 1, 15))
    subjects[1].sections.append(Section("Paper 1", 1, 15))
    subjects[2].sections.append(Section("Paper 1", 1, 15))

    timer_one = Timer(window, None, subjects, datetime.timedelta(hours=1, minutes=15))
    timer_one.frame.grid(column=0)

    timer_one.start_timer()
